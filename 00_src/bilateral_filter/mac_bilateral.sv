...
module mac_bilateral #(
  ...
  input  logic [8:0][WIDTH_PX-1:0]         px_i,
  input  logic [8:0][WIDTH_WC-1:0]         wc_i,
  output logic                             mac_valid_o,
  output logic [(WIDTH_MUL+GUARD_BIT)-1:0] mac_num_out,
  output logic [(WIDTH_WC+GUARD_BIT)-1:0]  mac_den_out
);
//========================DECLARATION========================================================================================
...
//========================STAGE 1: MULTIPLIER==================================================================================
  ...
  cla_adder_13bit adder_den_0 (.a_i(wc_i[0]),.b_i(wc_i[1]),.cin_i(1'b0),.result_o(den_accum[0]),.cout_o());
  cla_adder_13bit adder_den_1 (.a_i(wc_i[2]),.b_i(wc_i[3]),.cin_i(1'b0),.result_o(den_accum[1]),.cout_o());
  cla_adder_13bit adder_den_2 (.a_i(wc_i[4]),.b_i(wc_i[5]),.cin_i(1'b0),.result_o(den_accum[2]),.cout_o());
  cla_adder_13bit adder_den_3 (.a_i(wc_i[6]),.b_i(wc_i[7]),.cin_i(1'b0),.result_o(den_accum[3]),.cout_o());
  always_comb begin: mul_stage
    mul_add_next.op_mul           = op_mul;
    mul_add_next.den_wc8_delay    = wc_i[8];
...
//========================STAGE 4: FINAL RESULT========================
  ...
  always_ff @(posedge i_clk or negedge ni_rst) begin : final_out_register
    ...
    mac_num_out <= add_accum[7];
    mac_den_out <= den_accum[7];
    end
  end
endmodule